...
export const AdminProductForm = () => {
    ...
    // Image & description data per color
    ...
    // Temporary inputs for adding colors / sizes
    ...
    const {
        ...
        formState: { errors },
        reset,
    } = useForm<Omit<Product, 'id'> & { additionalImages?: string }>({
        defaultValues: {
            name: '',
            price: 0,
            marketPrice: 0,
    ...
    // Load categories and product data when editing
    useEffect(() => {
        const load = async () => {
            try {
                ...
                if (isEditing && id) {
                    ...
                    if (product.variants && product.variants.length > 0) {
                        ...
                        setGeneratedVariants(
                            product.variants.map((v) => ({ ...v, tempId: Math.random().toString(36).substr(2, 9) }))
                        );
                        // Extract per‑color images
                        const cImg: Record<string, string> = {};
                        const cAddImg: Record<string, string> = {};
                        uniqueColors.forEach((col) => {
                            const variant = product.variants?.find((v) => v.color === col);
                            if (variant?.images?.length) {
                                cImg[col] = variant.images[0];
                                cAddImg[col] = variant.images.slice(1).join(', ');
                            }
                        });
                        setColorImages(cImg);
                        setColorAdditionalImages(cAddImg);
                    }
                }
            } catch (e) {
                console.error(e);
    ...
    const handleGenerateVariants = () => {
        ...
        const findExisting = (c?: string, s?: string) =>
            generatedVariants.find((v) => v.color === c && v.size === s);
        if (hasColors && hasSizes) {
            colors.forEach((color) => {
                sizes.forEach((size) => {
                    const existing = findExisting(color, size);
                    newVariants.push(
                        existing || { color, size, stock_quantity: 0, tempId: Math.random().toString(36).substr(2, 9) }
                    );
                });
            });
        } else if (hasColors) {
            colors.forEach((color) => {
                ...
                newVariants.push(
                    existing || { color, stock_quantity: 0, tempId: Math.random().toString(36).substr(2, 9) }
                );
            });
        } else if (hasSizes) {
            sizes.forEach((size) => {
                ...
                newVariants.push(
                    existing || { size, stock_quantity: 0, tempId: Math.random().toString(36).substr(2, 9) }
                );
            });
        }
    ...
    // Auto‑update main image when first color changes
    useEffect(() => {
    ...
    // Handlers for adding / removing colors & sizes
    const addColor = () => {
    ...
    const removeColor = (color: string) => {
        ...
        const { [color]: _, ...remainingImages } = colorImages;
        setColorImages(remainingImages);
        const { [color]: __, ...remainingAddImages } = colorAdditionalImages;
        setColorAdditionalImages(remainingAddImages);
        const { [color]: ___, ...remainingDesc } = colorDescriptions;
        setColorDescriptions(remainingDesc);
        setGeneratedVariants((prev) => prev.filter((v) => v.color !== color));
    };
    ...
    const removeSize = (size: string) => {
        ...
        setGeneratedVariants((prev) => prev.filter((v) => v.size !== size));
    };

    // Submit handler – includes automated splitting when creating multi‑color products
    const onSubmit = async (data: any) => {
        try {
            setIsSubmitting(true);

            if (hasColors && colors.length > 1 && !isEditing) {
                ...
                for (const color of colors) {
                    const colorVariants = generatedVariants
                        ...
                        .map((v) => ({
                            ...
                            images: [
                                colorImages[color] || data.imageUrl,
                                ...(colorAdditionalImages[color]
                                    ? colorAdditionalImages[color].split(',').map((u: string) => u.trim()).filter(Boolean)
                                    : []),
                            ],
                        }));
                    const colorStock = colorVariants.reduce((sum, v) => sum + v.stock_quantity, 0);
                    const productData = {
                        ...data,
                        name: data.name,
                        slug: `${data.name.toLowerCase().replace(/ /g, '-').replace(/[^\\w-]+/g, '')}-${color.toLowerCase()}`,
                        description: colorDescriptions[color] || data.description,
                ...
                showAlert('Success', `Created ${colors.length} linked products successfully!`, 'success');
            } else {
                        ...
                        images:
                            hasColors && v.color
                                ? [
                                    ...
                                    ...(colorAdditionalImages[v.color]
                                        ? colorAdditionalImages[v.color].split(',').map((u: string) => u.trim()).filter(Boolean)
                                        : []),
                                ]
                                : [data.imageUrl],
                    }));
                ...
                <div className="bg-white rounded-xl shadow-sm p-6">
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none"
                                />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none bg-white"
                                    >
                            ...
                            <div>
                                <label className="block text-sm font-bold mb-1">
                                    Variant Group ID <span className="text-xs font-normal text-gray-500">(Optional)</span>
                                </label>
                                <div className="flex gap-2">
                                    <input
                                        {...register('colorVariantGroup')}
                                        className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none"
                                        placeholder="Paste ID to link products"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => {
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                    Paste the same ID here for different products (e.g. Red Shirt, Blue Shirt) to link them as color variants.
                                </p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-bold mb-1">Market Price (₹)</label>
                                    <input
                                        type="number"
                                        {...register('marketPrice', { min: 0 })}
                                        className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none"
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold mb-1">
                                        Cost Price (₹) <span className="text-xs font-normal text-gray-500">(Admin)</span>
                                    </label>
                                    <input
                                        type="number"
                                        {...register('costPrice', { min: 0 })}
                                        className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none bg-gray-50"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg border">
                                    ...
                                    <div className="flex gap-2 mb-3">
                                        ...
                                        <button
                                            type="button"
                                            onClick={addColor}
                                            className="bg-black text-white px-3 md:px-4 py-2 rounded font-bold shrink-0"
                                        >
                                            Add
                                        </button>
                                    </div>
                            ...
                            {hasSizes && (
                                <div className="bg-gray-50 p-4 rounded-lg border">
                                    ...
                                    <div className="flex gap-2 mb-3">
                                        <select value={sizeType} onChange={(e) => setSizeType(e.target.value)} className="border rounded p-2 bg-white shrink-0">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {sizes.map((s) => (
                                            <span key={s} className="bg-white border px-3 py-1 rounded-full text-sm flex items-center gap-2 shadow-sm">
                                                {s}{' '}
                                                <button type="button" onClick={() => removeSize(s)} className="hover:text-red-500">
                                                    <Icons.X className="w-3 h-3" />
                                                </button>
                                            </span>
                                        ))}
                            ...
                            {!hasColors && !hasSizes && (
                                <div>
                                    ...
                                    <input type="number" {...register('stock', { min: 0 })} className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none" />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {!hasColors && (
                                <div>
                                    <label className="block text-sm font-bold mb-1">Main Image URL</label>
                                    <input
                                        {...register('imageUrl', { required: !hasColors })}
                                        className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-black outline-none"
                                        placeholder="https://..."
                                    />
                                </div>
                                    ...
                                    {colors.map((color) => (
                                        <div key={color} className="border p-4 rounded-lg bg-gray-50">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div>
                                                    <label className="text-xs font-bold text-gray-500 uppercase">Description (Specific to {color})</label>
                                                    <textarea
                                                        value={colorDescriptions[color] || ''}
                                                        onChange={(e) => setColorDescriptions({ ...colorDescriptions, [color]: e.target.value })}
                                                        className="w-full border rounded p-2 text-sm"
                                                        placeholder={`Description for ${color} variant...`}
                                                        rows={2}
                                                    />
                                                </div>
                                            </div>
                            ...
                            {/* Additional images for non‑color products */}
                            {!hasColors && (
                                <div>
                                    <label className="block text-sm font-bold mb-1">Additional Images</label>
                                    <textarea
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <label htmlFor="trending" className="font-bold cursor-pointer">
                                Mark as Trending
                            </label>
                        </div>
                        ...
                        <div className="flex justify-end gap-4 pt-6 border-t">
                            ...
                            <button
                                ...
                                className="px-8 py-3 bg-black text-white rounded-xl font-bold hover:bg-gray-800 disabled:opacity-50"
                            >
